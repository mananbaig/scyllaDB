...
get_view_natural_endpoint(
    ...
    if (rack_aware_pairing && (network_topology->get_replication_factor(my_datacenter) % topology.get_datacenter_rack_nodes().at(my_datacenter).size()) == 0) {
        simple_rack_aware_pairing = true;
        size_t rack_rf = network_topology->get_replication_factor(my_datacenter) / topology.get_datacenter_rack_nodes().at(my_datacenter).size();
        for (const auto& [rack, nodes] : topology.get_datacenter_rack_nodes().at(my_datacenter)) {
    ...
    auto resolve = [&] (const locator::topology& topology, const locator::host_id& ep, bool is_view) -> const locator::node& {
        auto np = topology.find_node(ep);
        if (!np) {
            throw std::runtime_error(format("get_view_natural_endpoint: {} replica {} not found in topology", is_view ? "view" : "base", ep));
        } else if (np->left() || np->endpoint() == gms::inet_address{}) {
            vlogger.warn("get_view_natural_endpoint: {} replica {} already left the cluster", is_view ? "view" : "base", ep);
        }
        return *np;
    };
    ...
    // FIXME: With scylladb/scylladb#21781 we'd be able to use a single topology&
    // since base and view effective_replication_map:s would use the same
    // topology instance.
    ...
    if (base_it == base_endpoints.end()) {
        ...
        vlogger.warn("Could not find {} in base_endpoints={}", me,
                orig_base_endpoints | boost::adaptors::transformed([] (const locator::node& n) { return fmt::format("{}/{}", n.host_id(), n.endpoint()); }));
        return {};
    }
    ...
    if (idx >= view_endpoints.size()) {
        ...
        vlogger.warn("Could not pair {}: rack_aware={} base_endpoints={} view_endpoints={}", me,
                ...
                orig_base_endpoints | boost::adaptors::transformed([] (const locator::node& n) { return fmt::format("{}/{}", n.host_id(), n.endpoint()); }),
                orig_view_endpoints | boost::adaptors::transformed([] (const locator::node& n) { return fmt::format("{}/{}", n.host_id(), n.endpoint()); }));
        return {};
    }
