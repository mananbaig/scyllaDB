...
get_view_natural_endpoint(
    ...
    SCYLLA_ASSERT(std::abs((int)(base_endpoints.size() - view_endpoints.size())) <= 1);
        ...
        wait_for_all_updates wait_for_all)
{
    ...
    co_await max_concurrent_for_each(view_updates, max_concurrent_updates,
        ...
        if (extra_target_endpoint) {
            // The extra endpoint is a view replica whose paired base replica is not ready yet.
            // It's not pending, so it shouldn't be duplicated in the pending endpoints list.
