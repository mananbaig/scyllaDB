...
std::pair<view_ptr, cql3::cql_warnings_vec> create_view_statement::prepare_view(data_dictionary::database db) const {
    ...
    if (!boost::regex_match(std::string(cf_name), name_regex)) {
        throw exceptions::invalid_request_exception(format("\"{}\" is not a valid materialized view name (must be alphanumeric character only: [0-9A-Za-z]  or the underscore)", cf_name.c_str()));
    }
