...
async def test_read_repair_with_conflicting_hash_keys(request: pytest.FixtureRequest, manager: ManagerClient) -> None:
    ...
    4. Add with 2 rows that have primary keys causing a hash collision.
    ...
    6. Run a SELECT query with ALL consistency level, forcing reading from all 3 nodes.
       ...
       token is value is the same for both keys, this causes an incorrect diff calculation
    ...
    7. Run the same SELECT query with ALL consistency level, forcing reading from all 3 nodes.
       ...
       result of and prev step read repair. This value is newer than others, thus the resulting
       value will be incorrect in case the diff calculation algorithm is using a token as a key.
    ...
    # Validate the results (should be wrong)
    assert len(res) == 2
    for row in res:
        if (row.pk == pk1):
            assert row.c == 111
