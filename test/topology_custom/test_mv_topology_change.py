...
@pytest.mark.asyncio
@pytest.mark.parametrize("delayed_replica", ["base", "view"])
...
async def test_mv_rf_change(manager: ManagerClient, delayed_replica: str, altered_dc: str):
    ...
    await cql.run_async("CREATE TABLE ks.tab (pk int, ck int,  PRIMARY KEY (pk, ck))")
    await cql.run_async("CREATE MATERIALIZED VIEW ks.tabv AS SELECT pk, ck FROM ks.tab WHERE ck IS NOT NULL PRIMARY KEY (ck, pk)")

    # Insert a row so that there's data to be streamed during tablet migration
    await cql.run_async("INSERT INTO ks.tab (pk,ck) VALUES (0,0)")

    await asyncio.gather(*[manager.api.enable_injection(s.ip_addr, f"block_tablet_streaming", True, parameters={'delayed_replica': delayed_replica}) for s in servers])
    ...
    await cql.run_async(SimpleStatement("INSERT INTO ks.tab (pk,ck) VALUES (1,1)", consistency_level=ConsistencyLevel.ONE))
    ...
    res = await cql.run_async(SimpleStatement(f"SELECT * FROM ks.tabv", consistency_level=ConsistencyLevel.ONE))
    assert len(res) == 2
